...
df = (get_lps25_events()
        .set_tags(search_terms)
        .as_dataframe()
)
df['presentation_id'] = df.get('presentation_id', None)
df['authors'] = df.get('authors', None)
df['affiliations'] = df.get('affiliations', None)
df = df.sort_values(by=["start_dt", "session_id", "presentation_id"], ascending=[True, True, False], na_position="first")
